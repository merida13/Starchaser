...
function Coin(pos) {
  this.basePos = this.pos = pos.plus(new Vector(-0.2, -.4));
...
function Enemy(pos) {
  this.basePos = this.pos = pos.plus(new Vector(.2, -.9));
...
function Egg(pos) {
  this.basePos = this.pos = pos.plus(new Vector(0.2, -0.8));
  this.size = new Vector(1.5, 1.8);
  // Make it go back and forth in a sine wave.
  this.wobble = Math.random() * Math.PI * 2;
}
...
Player.prototype.moveX = function(step, level, keys) {
  ...
  if (keys.up) {
	Player.prototype.type = "playerjump"  
  }
